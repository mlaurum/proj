...
def apply_primitive(procedure, args_scheme_list, env):
    ...
    try:
        ...
        raise SchemeError("apply_primitive")
    # END Question 4
...
def make_call_frame(procedure, args, env):
    ...
    # BEGIN Question 12
    formal = procedure.formals
    if isinstance(procedure,LambdaProcedure):
        return procedure.env.make_child_frame(formal,args)
    elif isinstance(procedure,MuProcedure):
        return env.make_child_frame(formal,args)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 9A
        new=Pair(target.second, expressions.second)
        Lam = do_lambda_form(new, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        if scheme_symbolp(bindings.first.first):
            ...
            raise SchemeError("wrong binding")
...
def check_formals(formals):
    ...
    cache=[]
    while formals:
        if scheme_symbolp(formals.first) and formals.first not in cache:
            cache.append(formals.first)
            formals=formals.second
        else:
            raise SchemeError("check_formal")
    # END Question 11B
