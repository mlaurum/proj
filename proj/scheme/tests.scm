...
;;;q 5A
(define mnadsfbi3#@3wkjn 5)
;expects mnadsfbi3#@3wkjn
mnadsfbi3#@3wkjn
;expects 5
(define define 4)
;expects define
...
;;; q9
(define (the x) (* (quote nero) x))
;expect the
(the 3)
;expect nero nero nero
(define (stanford x) 
          (define (bear y) (* x y)))
;expect stanford
((stanford 4) 2)
...
;expect tis not
(if (not (eq? 'merry 'christmas)) (quote tis not) 'christmas)
;expect christmas
...
(cond
    ((= 4 2) 'tack)
    ((eq? orn Tiatn) 'erin)
    (else (cons 'answer (cons '= (cons 00101010)))))
;expect (answer = 00101010)
