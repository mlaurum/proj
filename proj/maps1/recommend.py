...
def group_by_centroid(restaurants, centroids):
    ...
    for r in restaurants:
        r_location = restaurant_location(r)
        groups.append([find_closest(r_location, centroids), r])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    x_average, y_average = mean(xs), mean(ys)
    s_xx = sum([(x - x_average) ** 2 for x in xs])
    s_yy = sum([(y - y_average) ** 2 for y in ys])
    s_xy = sum([(x - x_average) * (y - y_average) for x, y in zip(xs, ys)])
    b = s_xy / s_xx
