...
def solve(P, M, N, C, items, constraints):
  ...
  for i in items:
    ...
    if i[4] - i[3] > 0:
      heuristic = (i[3]-i[4])/(connections[i[1]]**0.3)
      priorityQ.put((heuristic,i))
    for k in range(int(math.log(N)-1)):
      ite = items[k]
      heuristicIte = (ite[3]-ite[4])/(connections[ite[1]]**0.3)
      availableItems[i[0]] = set()
      if heuristicIte > heuristic:
        availableItems[i[0]].add(ite)
  ...
  for sample in range(1000):
    rng = random.randint(0,len(itemlist)-1)
    toReplace = itemlist[rng]
    replace = toReplace
    for i in availableItems[toReplace[0]]:
      if i[2] <= toReplace[2] and i[3]<=toReplace[3] and i[4] >= replace[4]:
        print("newItem: ", i)
        replacedHue = (toReplace[4]-toReplace[3])
        newItemHue = (i[4]-i[3])
        replace = newItemHue
        print("old Profit was: ",  replacedHue,"New profit is: ", newItemHue)
  return range(0,1)
