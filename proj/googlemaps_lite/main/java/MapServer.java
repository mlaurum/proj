...
public class MapServer {
    ...
    private static LinkedList<Long> neededIds;
    private static boolean routeReq;
    ...
    public static Map<String, Object> getMapRaster(Map<String, Double> params, OutputStream os) {
        try {
            ...
            ArrayList<Node> images = qt.rasterIm(params.get("ullon"), params.get("ullat"),
                    params.get("lrlon"), params.get("lrlat"), params.get("w"), params.get("h"));
            int numW = (int) images.get(images.size() - 2).ullon;
            int numH = (int) images.get(images.size() - 1).ullon;
            int width = numW * 256, height = numH * 256;

            double ullon = images.get(0).ullon;
            double ullat = images.get(0).ullat;
            double lrlon = images.get(images.size() - 3).lrlon;
            double lrlat = images.get(images.size() - 3).lrlat;
            double dppH = (lrlat - ullat) / height;
            double dppW = (lrlon - ullon) / width;

            rasteredImageParams.put("raster_ul_lon", ullon);
            rasteredImageParams.put("raster_ul_lat", ullat);
            rasteredImageParams.put("raster_lr_lon", images.get(images.size() - 3).lrlon);
            rasteredImageParams.put("raster_lr_lat", images.get(images.size() - 3).lrlat);
            rasteredImageParams.put("raster_width", width);
            rasteredImageParams.put("raster_height", height);
            rasteredImageParams.put("depth", images.get(images.size() - 3).depth);
            rasteredImageParams.put("query_success", true);



            BufferedImage result = new BufferedImage(width, height, //work these out
                    BufferedImage.TYPE_INT_RGB);
            Graphics grap = result.getGraphics();
            int x = 0, y = 0;
            ...
            for (Node image : images) {
               ...
               //System.out.println(image.imageTitle);
                BufferedImage bi = ImageIO.read(new File(IMG_ROOT  + image.imageTitle));

                grap.drawImage(bi, x, y, null);
                x += 256;
            ...
            if (routeReq) {
               // System.out.println("reached inside: " + neededIds.size());

                Stroke bs = new BasicStroke(MapServer.ROUTE_STROKE_WIDTH_PX,
                        BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
                ((Graphics2D) grap).setStroke(bs);
                grap.setColor(MapServer.ROUTE_STROKE_COLOR);

                for (int i = 0; i < neededIds.size() - 1; i++) {
                    grap.drawLine((int) ((g.vertices().get(
                            (double) neededIds.get(i)).lon() - ullon) / dppW),
                            (int) ((g.vertices().get(
                                    (double) neededIds.get(i)).lat() - ullat) / dppH),
                            (int) ((g.vertices().get(
                                    (double) neededIds.get(i + 1)).lon() - ullon) / dppW),
                            (int) ((g.vertices().get(
                                    (double) neededIds.get(i + 1)).lat() - ullat) / dppH));
    ...
    public static LinkedList<Long> findAndSetRoute(Map<String, Double> params) {
        GraphNode smallest = new GraphNode();
        GraphNode largest = new GraphNode();

        for (GraphNode j : GraphDB.vertices().values()) {
            double curlengths = j.euclidianDist(params.get("start_lat"), params.get("start_lon"));
            double curlengthh = j.euclidianDist(params.get("end_lat"), params.get("end_lon"));
            if (curlengths < smallest.euclidianDist(params.get("start_lat"),
                    params.get("start_lon"))) {
                smallest = j;
            }
            if (curlengthh < largest.euclidianDist(params.get("end_lat"), params.get("end_lon"))) {
                largest = j;
            }
        }
        ASTAR astarClass = new ASTAR(smallest, largest);
        neededIds = astarClass.aStar();

        for (GraphNode j : GraphDB.vertices().values()) { //this?
            j.setParent(null);
        }
    ...
    public static List<String> getLocationsByPrefix(String prefix) {
       /* prefix = g.cleanString(prefix);
        String tmp = prefix;
        System.out.println("test PREFIC: " + prefix);
        return g.Gr.getLocation(prefix);*/
    ...
    public static List<Map<String, Object>> getLocations(String locationName) {
        /*locationName = g.cleanString(locationName);
        return g.Gr.searchTitle(locationName);*/
        return new ArrayList<>();
    }
}
