...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 5A
        second = scheme_eval(expressions.second.first, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        check_form(clause.second, 1, 1) #you can only bind one value to a formal
...
def check_formals(formals):
    ...
    while formals:
        if not scheme_symbolp(formals.first):
            raise SchemeError(formals.first, "is not a symbol")
        if formals.first in seen:
            raise SchemeError(formals.first, "is repeated")
